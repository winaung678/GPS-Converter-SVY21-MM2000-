    ...
    inputs.forEach(input => {
        if (input.id.includes('_lat_') || input.id.includes('_lon_') || input.id.includes('_inp_') || input.id.includes('so_m_')) {
            input.addEventListener('paste', function(e) {
                e.preventDefault(); let pasteData = (e.clipboardData || window.clipboardData).getData('text');
                let cleanNumber = pasteData.replace(/[^\d.-]/g, ''); document.execCommand('insertText', false, cleanNumber);
            });
        }
    });
}
...
window.handleTopMenuClick = function() {
    window.isTopoMode = !window.isTopoMode;
    window.updateTopoUI();
};

window.updateTopoUI = function() {
    let btn = document.getElementById('topMenuBtn');
    ...
    if (window.isNativeGPSActive && (n === 1 || n === 2 || n === 5)) {
        ...
        if (window.isTopoMode) {
            btn.innerHTML = "🧮 Data View";
            btn.classList.add('topo-active');
            document.getElementById('shared_map_view').classList.remove('hidden');
            document.getElementById('v2_calc_ui').classList.add('hidden');
            document.getElementById('m_calc_ui').classList.add('hidden');
            document.getElementById('g_calc_ui').classList.add('hidden');
            ...
            btn.classList.remove('topo-active');
            document.getElementById('shared_map_view').classList.add('hidden');
            if (n === 1) document.getElementById('v2_calc_ui').classList.remove('hidden');
            if (n === 2) document.getElementById('m_calc_ui').classList.remove('hidden');
            if (n === 5) document.getElementById('g_calc_ui').classList.remove('hidden');
        }
    } else {
...
window.switchApp = function(n) {
    ...
    // 🔴 တခြား Tab သို့ ကူးပြောင်းချိန်တွင် GPS ပွင့်နေပါက အလိုအလျောက် ပိတ် (Off) ပေးမည်
    if (window.isNativeGPSActive && window.activeApp !== n) {
    ...
    window.activeApp = n;
// ... (အောက်ဘက်ရှိ ကျန်သည့် Code များ အားလုံး အရင်တိုင်း ထားပါ)
    ...
    let menuBtn = document.getElementById('topMenuBtn');

    // 🔴 GPS နှင့် Menu Button Logic (App 4 / COGO တွင် အစပိုင်း၌ ဖျောက်ထားမည်)
    if (n === 0 || n === 4) {
    ...
    if (n === 3 || n === 6) {
        window.isTopoMode = false;
        menuBtn.classList.add('hidden');
        document.getElementById('shared_map_view').classList.remove('hidden');
        let mapDiv = document.getElementById('map_view');
        if (n === 6) { mapDiv.classList.add('map-full-height'); } else { mapDiv.classList.remove('map-full-height'); }
    } else if (n === 4) {
        // 🔴 COGO သို့ဝင်လျှင် COGO Main Menu သို့ Reset ချပေးမည်
        window.isTopoMode = false;
        menuBtn.classList.add('hidden');
        document.getElementById('shared_map_view').classList.add('hidden');
        document.getElementById('map_view').classList.remove('map-full-height');
        if (typeof window.closeCogoTool === "function") {
    ...
    if(csvSelect) {
        if (n === 5) { csvSelect.options[0].text = `UTM (PNEZD) → WGS84 (Lat, Lon)`; csvSelect.options[1].text = `WGS84 (Lat, Lon) → UTM (PNEZD)`; csvSelect.options[2].style.display = 'none'; if (csvSelect.value === "3") csvSelect.value = "1"; }
        else { let dName = (n === 1) ? "SVY21" : "MM2000"; csvSelect.options[0].text = `${dName} (Local NE) → WGS84 LL & NE`; csvSelect.options[1].text = `WGS84 LL (Lat, Lon) → ${dName} NE`; csvSelect.options[2].text = `WGS84 NE → ${dName} Local NE`; csvSelect.options[2].style.display = 'block'; }
    }
    ...
    if(n !== 0 && n !== 7) {
        ...
        setTimeout(() => {
            ...
            if (window.rawDxfEntities && window.rawDxfEntities.length > 0 && window.dxfLineLayer === null) {
                window.renderDXF();
            } else {
...
window.toggleGlobalGPS = function() {
    ...
    if(window.isNativeGPSActive) {
        ...
        if (typeof window.AndroidNative === 'undefined' && navigator.geolocation) { window.webGPSWatchId = navigator.geolocation.watchPosition((pos) => { window.onNativeGPSUpdate({ lat: pos.coords.latitude, lon: pos.coords.longitude, alt: pos.coords.altitude||0, acc: pos.coords.accuracy||0, hAcc: pos.coords.accuracy||0, vAcc: pos.coords.altitudeAccuracy||0, satCount: 5 }); }, (err) => {}, { enableHighAccuracy: true, maximumAge: 0 }); }
    } else {
...
function fetchGeoidDataFromNative(lat, lon) {
    if (window.AndroidNative && window.AndroidNative.getGeoidData) {
        ...
        try { let data = JSON.parse(jsonStr); if (data.geoid_n !== null) { window.currentGeoidN = data.geoid_n; window.currentGeoidModel = data.geoid_model; } else { window.currentGeoidN = 0; window.currentGeoidModel = "Unknown"; } }
        catch(e) { window.currentGeoidN = 0; window.currentGeoidModel = "Unknown"; }
    ...
    if(window.activeApp !== 0 && window.activeApp !== 7 && window.leafletMap) {
        if(!window.markerCurrent) {
            let currIcon = L.divIcon({
                ...
                iconSize: [50, 50],
                iconAnchor: [25, 25],
                html: `<div id="gps-heading-cone" style="width: 50px; height: 50px; transform: rotate(${window.compassAzimuth||0}deg); transform-origin: center center; transition: transform 0.1s ease-out;">
                          <svg width="50" height="50" viewBox="0 0 50 50">
                              <path d="M25,25 L10,0 A25,25 0 0,1 40,0 Z" fill="url(#gradCone)" opacity="0.5" />
                              <circle cx="25" cy="25" r="6" fill="#2563eb" stroke="white" stroke-width="2.5" />
                              <defs>
                                  <linearGradient id="gradCone" x1="0%" y1="100%" x2="0%" y2="0%">
